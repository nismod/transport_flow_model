...
def main(config):
    ...
    network_data_folder = os.path.join(processed_data_path,"/path_to_network_data/")
    network_data_folder = output_data_path
    ...
    # Read the network file with the topology. Assuming here that it is a parquet file
    network_dataframe = gpd.read_parquet(os.path.join(network_data_folder,"network_file.parquet"))
    ...
    od_dataframe = pd.read_parquet(os.path.join(flow_od_folder,"od_file.parquet"))
    ...
    n_df = network_dataframe[
                        ...
                        network_id_column,network_capacity_column,
                        flow_column,network_cost_column
                        ] + network_attribute_columns
                        ]
    flow_routes, unassinged_routes, n_df = od_flow_allocation_capacity_constrained(
                                                od_df,n_df,
                                                flow_column,network_cost_column,
                                                network_id_column,
    ...
    n_df.to_parquet(os.path.join(results_folder,"network_edge_total_flows.parquet"))
    ...
    if len(unassinged_routes) > 0:
        ...
        unassinged_routes.to_parquet(os.path.join(results_folder,"unassigned_od_flows.parquet"))
    ...
    if len(flow_routes) > 0:
        ...
        flow_sub_columns = [c for c in od_dataframe.columns.values.tolist() if c not in [
                            origin_id_column,destination_id_column,flow_column]
                            ]
        if len(flow_sub_columns) > 0:
            od_dataframe.rename(columns={flow_column:"initial_flow"},inplace=True)
            flow_routes = pd.merge(flow_routes,
                            od_dataframe,
                            how="left",on=[origin_id_column,destination_id_column])
            flow_routes[flow_sub_columns] = flow_routes[
                                                flow_sub_columns].multiply(flow_routes[
                                                    flow_column]/flow_routes["initial_flow"],axis="index")
            flow_routes.drop("initial_flow",axis=1,inplace=True)
        
        flow_routes.to_parquet(
                os.path.join(results_folder,"od_flows.parquet"),
                index=False)
