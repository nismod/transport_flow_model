...
def main(config):
    ...
    failure_edges = pd.read_parquet(
        os.path.join(damages_results_path, "failure_set.parquet")
    )
    ...
    edge_path_idx = get_flow_paths_indexes_and_edges_dataframe(
        flow_df, edge_path_column, id_column=network_id_column
    )
    ...
    for row in failure_edges.itertuples():
        ...
        if network_df[network_df["id"].isin(fail_edges)][flow_column].sum() > 0:
            ...
            if len(rerouted_flows) > 0:
                ...
                rerouting_loss = pd.DataFrame(
                    [0] * len(rerouting_loss_columns), columns=rerouting_loss_columns
                )
            ...
            if len(isolated_flows) > 0:
                ...
                isolation_loss = pd.DataFrame(
                    [0] * len(flow_value_columns), columns=flow_value_columns
                )
        ...
        sum_columns = [
            c for c in ef_list.columns.values.tolist() if c != failure_id_column
        ]
        ef_list = (
        ...
        ef_list["total_flow_loss"] = (
            ef_list[f"rerouting_{cost_column}"] + ef_list[flow_value]
        )
        ef_list.to_csv(
            os.path.join(results_folder, "flow_disruption_losses.csv"), index=False
        )
